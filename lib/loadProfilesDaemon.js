...
function isProfileAlreadyCreated(profileName, loginUserName) {
    try {
        ...
        // ENOENT is the answer to the question, not a failure: this function exists
        // to ask whether the profile has been imported yet, and on a fresh base dir
        // the answer is "no" for every profile in ~/.aws. Logging that at ERROR
        // printed one alarming line per profile during a completely successful
        // startup — 24 of them here — and then created each profile fine. Anything
        // else (a permission problem, a path that is a directory) is a real fault
        // and still gets reported.
        if (e && e.code === 'ENOENT') {
            log.debug('profile ', profileName, ' not imported yet — will be created.');
        } else {
            log.warn('could not check whether profile ', profileName, ' exists: ', e.message);
        }
        return false;
    }
}
