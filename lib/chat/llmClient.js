 ...
 * LLM_API_KEY, one cached OpenAI client for the life of the process. It is now
 * per-user and per-request, because the provider, key and model are settings the
 * user changes in the UI while the server is running — and a process-lifetime
 ...
 *
 * Resolution order (lib/llm/llmSettings.resolveActive): stored settings first,
 * then LLM_API_KEY/LLM_MODEL/LLM_BASE_URL. The environment is retained as a
 * fallback rather than removed — an existing install, and an air-gapped deploy
 * that only has environment variables, must keep working across this upgrade —
 * but it can no longer override what the user chose in Settings.
 *
 * config.properties [llm] still supplies the *behavioural* knobs that are not
 * per-provider: reasoning effort, temperature, tool-iteration cap.
 */
