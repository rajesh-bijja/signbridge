...
async function resolveChatSession(req, res) {
    ...
    if (!session.ok) {
        res.status(session.statusCode || 503).json({
            ...
            // True for every reason, including 'disabled'. It used to be excluded,
            // because "disabled" meant an operator had set llm.enabled=false in a
            // config file and pointing the user at Settings would have been a dead
            // end. The switch lives in Settings now, so every way a turn can fail
            // to start is something the user can fix there.
            needsLlmSetup: true
        });
        return null;
    }
