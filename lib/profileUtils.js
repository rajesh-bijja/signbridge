...
// Profiles hold IAM keys, SSH private keys, bearer tokens, and cached SSO state.
const JSON_WRITE_OPTS = { spaces: 2, mode: 0o600 };
                ...
                coreUtils.searchOrCreateBasicSettingsArtifacts(loginUserName, (err, settingsReadObj, settingsAbsoluteDir, settingsMetadataFile) => {
                    if (err) {
                        ...
                        if (settingsOutputObj['tinyUrlIntegration']['isEnabled'] &&
                            ...
                            jsonfile.writeFile(settingsMetadataFile, settingsOutputObj, JSON_WRITE_OPTS, (writeErr) => {
                                if (writeErr) {
...
function createProfile(profileInputObj, cb) {
    ...
    coreUtils.getOrCreateProfilesDir(loginUserName, profileDir, (err, profilesAbsoluteDir) => {
        if (err) {
            ...
            jsonfile.writeFile(profileFile, profileOutputObj, JSON_WRITE_OPTS, (writeErr) => {
                if (writeErr) {
...
function getPublicClientCreds(region, cb) {
    ...
    if (!fs.existsSync(credsFile)) {
        let credsInputObj = {}
        jsonfile.writeFile(credsFile, credsInputObj, JSON_WRITE_OPTS, (writeErr) => {
            if (writeErr) {
    ...
    if (fs.existsSync(credsFile)) {
        ...
        jsonfile.readFile(credsFile, UTF8, (readErr, credsReadObj) => {
            if (readErr) {
                ...
                jsonfile.writeFile(credsFile, credsOutputObj, JSON_WRITE_OPTS, (writeErr) => {
                    if (writeErr) {
