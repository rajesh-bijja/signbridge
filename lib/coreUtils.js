...
// Owner-only for everything written under ~/.signbridge. History and favorites
// records are whole requests and responses — Authorization headers, bearer
// tokens, bodies — and the settings file carries the TinyURL integration token.
// jsonfile's default is 0644, so without this they are world-readable.
//
// The mode is honoured only when the file is CREATED; rewriting an existing file
// keeps its old mode. An install whose artifacts predate this therefore stays
// wide until docker-entrypoint.sh repairs it, which is why that pass exists.
// PRETTY is for the call sites that already pass spaces — jsonfile 6 ignores the
// legacy `jsonfile.spaces` global, so adding it elsewhere would reformat files
// that are compact today for no reason.
const OWNER_ONLY = { mode: 0o600 };
const OWNER_ONLY_PRETTY = { spaces: 2, mode: 0o600 };
...
function createHistoryDetailsForTheGivenRequest(executionDetailsAbsoluteFileName, completeRequestData, responseStatusCode, responseHeaders, responseData, cb) {
    ...
    jsonfile.writeFile(executionDetailsAbsoluteFileName, completeObject, OWNER_ONLY, function (err) {
        if(err) {
            return cb(err);
        } else {
...
function importCollectionForUser(userName, collectionName, collectionPayload, cb) {
    ...
    searchOrCreateBasicCollectionArtifacts(userName, timestamp, (err, collectionsAbsoluteDir, collectionsInputAbsoluteDir, collectionsOutputAbsoluteDir) => {
        ...
        fs.writeFile(inputCollectionFileName, serialized, OWNER_ONLY, (writeErr) => {
            if (writeErr) {
                return cb(writeErr);
            }
    ...
    try {
        ...
        getOrCreateCollectionsBaseOutputDir(loginUserName, (err, collectionsBaseOutputAbsoluteDir) => {
            ...
            recursive(collectionsBaseOutputAbsoluteDir, ["*.html", "*.txt", "*.readme", "*.test", ignoreFunc], function (err, filesList) {
                ...
                jsonfile.writeFile(targetMetadataFile, updatedMetadata, OWNER_ONLY_PRETTY, (writeErr) => {
                    if (writeErr) {
...
function updateHistoryMetadata(loginUserName, historyMetadataOutputObj, historyMetadataFile, cb) {
    jsonfile.writeFile(historyMetadataFile, historyMetadataOutputObj, OWNER_ONLY, function (err) {
        if(err) {
            return cb(err);
        } else {
...
function updateFavoritesMetadata(loginUserName, favoritesMetadataOutputObj, favoritesMetadataFile, cb) {
    jsonfile.writeFile(favoritesMetadataFile, favoritesMetadataOutputObj, OWNER_ONLY, function (err) {
        if(err) {
            return cb(err);
        } else {
...
function createCollectionsMetadata(loginUserName, collectionsMetadataOutputObj, collectionsMetadataFile, cb) {
    jsonfile.writeFile(collectionsMetadataFile, collectionsMetadataOutputObj, OWNER_ONLY, function (err) {
        if(err) {
            return cb(err);
        } else {
...
function createCollectionsMetadataAndRequests(loginUserName, timestamp, inputCollectionFileName, importedCollectionName, collectionsOutputAbsoluteDir, cb) {
    ...
    for (let count = 0; count < result.length; count++) {
        ...
        jsonfile.writeFileSync(requestFile, requestDetails, OWNER_ONLY);
    }
        ...
        getOrCreateHistoryDir(loginUserName, (err, historyAbsoluteDir) => {
            if (err) {
                ...
                if (fs.existsSync(historyMetadataFile)) {
                    ...
                    jsonfile.writeFile(historyMetadataFile, historyMetadataOutputObj, OWNER_ONLY, (writeErr) => {
                        if (writeErr) {
        ...
        getOrCreateFavoritesDir(loginUserName, (err, favoritesAbsoluteDir) => {
            if (err) {
                ...
                if (fs.existsSync(favoritesMetadataFile)) {
                    ...
                    jsonfile.writeFile(favoritesMetadataFile, favoritesMetadataOutputObj, OWNER_ONLY, (writeErr) => {
                        if (writeErr) {
        ...
        getOrCreateSettingsDir(loginUserName, (err, settingsAbsoluteDir) => {
            if (err) {
                ...
                if (fs.existsSync(settingsMetadataFile)) {
                    ...
                    jsonfile.writeFile(settingsMetadataFile, settingsMetadataOutputObj, OWNER_ONLY_PRETTY, (writeErr) => {
                        if (writeErr) {
