...
let log = require('./logger').create('ssoS3Post');
...
function callSsoS3Post(options, roleCredentials, cb) {
    ...
    if (options['body'] && options['headers'] && options['headers']['content-type'] &&
        ...
        if (options['isPayloadBase64Binary']) {
            payload = options['body'];
            let searchString = ';base64,'
            let indexOfBase64Header = payload.indexOf(';base64,');
            if (indexOfBase64Header == -1) {
                log.debug('base64 header is not present, not decoding and not recreating the payload.');
            } else {
                let length = indexOfBase64Header + searchString.length;
                payload = payload.substring(length);
                // Decode straight to a Buffer. This used to write the decoded
                // bytes to a temp file and read them back, which produced an
                // identical Buffer while putting the user's upload payload on
                // disk in /tmp for no reason.
                payload = Buffer.from(payload, "base64");
            }
        } else {
            log.debug('payload is not considered as base64 for the binary. Skipping decoding and recreating the payload.');
        }
    }
    ...
    log.debug('after analysis payload length is: ' + payload.length);
    let hash = crypto.createHash('md5');
    let hashDataDigest= hash.update(payload, 'utf-8').digest();
    let bufferObj = Buffer.from(hashDataDigest);
    let contentMd5 = bufferObj.toString('base64');
    ...
    invokeAwsApi(headers, payload, options['method'].toUpperCase(), options['path'], (awsApiErr, responseStatusCode, responseHeaders, data) => {
        if (awsApiErr) {
            return cb(awsApiErr);
        }
...
function invokeAwsApi(headers, payload, method, path, cb) {
    let host = headers['host'];
    delete headers['host'];
    log.debug('passed headers: [', redact.forLog(headers), '] payload: [', payload, '] host: [', host, '] path: ', path);
    log.debug('in final invocation payload length is :' + payload.length);
    log.debug('in final invocation headers :', redact.forLog(headers));
    let options = {
        'method': method,
        'protocol': 'https:',
        'hostname': host,
    ...
    let req = https.request(options, (res) => {
        ...
        res.on("end", () => {
            if (responseBody) {
                try {
                    log.debug('aws api invocation response status: ', res.statusCode);
                    if (res.statusCode == 200 || res.statusCode == 201) {
                        log.debug('success response from SSO aws api invocation ');
                        try {
                        ...
                        let errMsg = 'Error in aws api invocation: ' + res.statusCode + responseBody
                        log.error(errMsg);
                        return cb(new Error(errMsg));
                    }
                } catch (err) {
                    log.error('error occurred in extracting the response from aws api result: ', err.message);
                    return cb(err);
                }
            } else {
                let data = {};
        ...
        res.on("error", (error) => {
            if (error instanceof Object) {
                log.error('error response received in aws api invocation call: ', JSON.parse(error));
            } else {
                log.error('error response received in aws api invocation call : ', error);
            }
            return cb(error);
        });
    });
    ...
    req.on('socket', function (socket) {
        ...
        socket.on('timeout', function() {
            let message = 'socket timeout error occurred while trying to invoke aws api : ' + options['protocol'] + '//' + options['hostname'] + ':' + options['port'] +  options['path'];
            log.warn(message);
            req.abort();
        });
    });
    ...
    req.on('error', function(err) {
        let message = 'Error occurred while trying to invoke aws api : ' +
