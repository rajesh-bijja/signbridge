...
function invokeCommand(req, res) {
    ...
    if (options['authnMode'] === 'sso_user') {
        coreUtils.getOrCreateScriptsDir(options['userName'], (scriptDirErr, scriptsDir) => {
            ...
            // 0700, not 0755: only this process ever executes it, and the command text
            // can carry a secret in an argument (`ssm put-parameter --value ...`).
            fs.writeFileSync(scriptFile, commandPayload, { mode: 0o700 });
            ...
            profileUtils.searchProfile(options['userName'], profileName, (profileReadErr, profileReadObj) => {
                ...
                ssoUtils.getSecurityToken(profileReadObj, (credErr, roleCredentials) => {
                    ...
                    // Owner-only: this file is the live SSO portal token, which is
                    // exchangeable for fresh role credentials. It is also not our file —
                    // it is the AWS CLI's own cache, and the CLI writes it 0600. Note the
                    // mode applies only when the file does not exist yet; an already-wide
                    // one keeps its mode, which is why docker-entrypoint.sh repairs too.
                    fs.writeFile(credentialsFilePath, accessTokenCredsObj, { mode: 0o600 }, writeErr => {
                        if (writeErr) {
    ...
    credentialProvider.resolveByProfileName(options['userName'], options['profileName'], options['authnMode'],
        (credErr, resolved) => {
            ...
            coreUtils.getOrCreateScriptsDir(options['userName'], (scriptDirErr, scriptsDir) => {
                ...
                let scriptFile = scriptsDir + '/' + new Date().getTime();
                // Owner-only and owner-executable; see the note on the sso_user path above.
                fs.writeFileSync(scriptFile, commandPayload, { mode: 0o700 });
                fs.chmodSync(scriptFile, 0o700);
