...
let log = require('./logger').create('iamGet');
...
function callIamGet(options, iamUserCredentials, cb) {
    ...
    log.debug('complete options: ', options, ' and canonicalUriPath: [', canonicalUriPath, '], canonicalQueryString: ', canonicalQueryString);
    ...
    invokeIamAwsGetApi(headers, path, options['method'].toUpperCase(), (awsApiErr, responseStatusCode, responseHeaders, data) => {
        if (awsApiErr) {
            return cb(awsApiErr);
        }
...
function invokeIamAwsGetApi(headers, path, method, cb) {
    ...
    let req = https.request(options, (res) => {
        ...
        res.on("end", () => {
            if (responseBody) {
                try {
                    log.debug('iam aws GET api invocation response status: ', res.statusCode);
                    if (res.statusCode == 200 || res.statusCode == 201) {
                        log.debug('success response from iam aws GET api invocation');
                        try {
                        ...
                        let errMsg = 'Error in iam aws GET api invocation: ' + res.statusCode + responseBody
                        log.error(errMsg);
                        return cb(new Error(errMsg));
                    }
                } catch (err) {
                    log.error('error occurred in extracting the response from iam aws GET api result: ', err.message);
                    return cb(err);
                }
            } else {
                let data = {};
        ...
        res.on("error", (error) => {
            if (error instanceof Object) {
                log.error('error response received in iam aws GET api invocation call: ', JSON.parse(error));
            } else {
                log.error('error response received in iam aws GET api invocation call : ', error);
            }
            return cb(error);
        });
    });
    ...
    req.on('socket', function (socket) {
        ...
        socket.on('timeout', function() {
            let message = 'socket timeout error occurred while trying to invoke iam aws GET api : ' + options['protocol'] + '//' + options['hostname'] + ':' + options['port'] +  options['path'];
            log.warn(message);
            req.abort();
        });
    });
    ...
    req.on('error', function(err) {
        let message = 'Error occurred while trying to invoke iam aws GET api : ' +
