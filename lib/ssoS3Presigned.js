...
let log = require('./logger').create('ssoS3Presigned');
...
function callS3SsoPresigned(options, roleCredentials, cb) {
    ...
    // EARLIER of X-Amz-Expires and the session token's own expiration. Cap it to
    // the credential's real remaining life (expiration is epoch milliseconds) so
    // the URL doesn't advertise a lifetime it can't honour.
...
module.exports = {
    callS3SsoPresigned: callS3SsoPresigned
}