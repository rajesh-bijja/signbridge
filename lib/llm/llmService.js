 ...
 * registry, the saved settings, and — importantly — which source is actually in
 * effect. That last part is why `active` is here: with the environment still
 * supported as a fallback, "why is chat using gpt-5 when I selected Claude?" has
...
function updateLlmSettings(req, res) {
    ...
    llmSettings.load(userName, function (err, settings) {
        ...
        // which is how the UI falls back to the environment again.
