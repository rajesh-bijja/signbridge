 ...
 * and is the *only* source of a provider credential. There is no environment
 * fallback and no key in config.properties: a provider key is a secret, and the
 * one place it may live is here, sealed, where the app can verify it, mask it and
 * scope it to the provider the user chose. An install with nothing configured
 * does not chat — it tells the user to configure a provider (see resolveActive).
 *
