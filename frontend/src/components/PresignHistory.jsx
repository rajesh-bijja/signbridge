...
function PresignHistory() {
  ...
  // "Prompt Request Deletion in History" setting; defaults to true when unset.
  ...
  useEffect(() => {
    ...
    const displayHistoryProgressEvent = 'event_display_request_details_' + userName;
    
    // Listen for history details
    socket.on(displayHistoryProgressEvent, (obj) => {
      if (selectedHistoryIdRef.current === obj.historyId) {
        ...
        if (overallTextJsonObj && overallTextJsonObj.request && overallTextJsonObj.response) {
          // Update selected history with full details
    ...
    return () => {
      ...
      socket.off(displayHistoryProgressEvent);
    };
  }, []);
  ...
  const loadDeletePrompt = async () => {
    try {
      ...
      setPromptOnDelete(settings?.shouldPromptHistoryDeletion ?? true);
    } catch {
  ...
  const loadHistory = async (label = null) => {
    try {
      const data = await presignApi.populateHistoryDetails(label);
      if (data && data.length > 0) {
        const formattedData = data.map((item, index) => ({
          id: index + 1,
          historyId: item.executionDetailsFileName?.replace(".json", "") || "",
          method: item.method || "",
  ...
  // Load and show the details for a single history item in the right panel.
  const showHistoryDetails = (item) => {
    setSelectedHistory(item);
    setRequestLabel(item.requestLabel || "");
    selectedHistoryIdRef.current = item.historyId;
    setLoading(true);
    setError(null);
    setResponseData("Loading request details...");
    ...
    if (socket) {
      ...
      socket.emit('event_get_history_id_log', eventObj);
    } else {
  ...
  // clear the detail panel and offer bulk actions instead.
  const handleSelectionChange = (items) => {
    setSelectedItems(items);
    if (items.length === 1) {
      showHistoryDetails(items[0]);
    } else {
      setSelectedHistory(null);
  ...
  const handleReRun = async () => {
    if (!selectedHistory) {
      setError("Please select a history item to re-run");
      return;
    }
    ...
    try {
      // Re-run logic would go here
      // This would require fetching the full request details and invoking it
      setSuccess("Request re-run initiated. Check the response panel.");
    } catch (err) {
      setError(err.response?.data?.message || err.message || "Failed to re-run request");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePinToDashboard = async () => {
    if (!selectedHistory) {
      setError("Please select a history item to pin");
      return;
    }
    ...
    try {
      // Get the full request details from selectedHistory
      const requestData = selectedHistory.request;
      if (!requestData) {
  ...
  const handleDelete = async () => {
    if (selectedItems.length === 0) {
      setError("Please select one or more history items to delete");
      return;
    }
      ...
      const confirmed = await confirm({
        ...
        message:
          count === 1
            ...
            : `Are you sure you want to delete these ${count} history entries?`
      });
      if (!confirmed) {
        return;
      }
    ...
    try {
      // Delete each selected entry; collect any failures so partial success is
      // reported rather than swallowed.
      const failures = [];
      for (const item of selectedItems) {
        try {
          await presignApi.deleteHistoryDetails(item.historyId);
        } catch (err) {
          failures.push(item.historyId?.substring(0, 8) || item.historyId);
        }
      }
      const deleted = selectedItems.length - failures.length;
      if (failures.length === 0) {
        setSuccess(`Deleted ${deleted} history ${deleted === 1 ? "entry" : "entries"} successfully`);
      } else {
      ...
      setError(err.response?.data?.message || err.message || "Failed to delete history");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddToFavorites = async () => {
    if (selectedItems.length === 0) {
      setError("Please select one or more history items to add to favorites");
      return;
    }
    ...
    try {
      // Determine which selected items are already favorites so we can report
      // skips. Adding is idempotent server-side (already-present ids are a
      // no-op), so this is purely for an accurate message.
      let existingFavoriteIds = new Set();
      try {
        const favorites = await presignApi.populateFavoriteDetails(null);
        (favorites || []).forEach((fav) => {
          const favId = fav.executionDetailsFileName?.replace(".json", "");
          if (favId) existingFavoriteIds.add(favId);
        });
      } catch {
        // If we can't read favorites, proceed anyway — the add is still idempotent.
      }
      ...
      const failures = [];
      for (const item of selectedItems) {
        if (existingFavoriteIds.has(item.historyId)) {
          skipped++;
          continue;
        }
        try {
          await presignApi.addHistoryToFavorites(item.historyId);
          added++;
        } catch (err) {
          failures.push(item.historyId?.substring(0, 8) || item.historyId);
        }
      ...
      if (skipped) parts.push(`skipped ${skipped} already in favorites`);
      if (failures.length === 0) {
        setSuccess(`Favorites updated: ${parts.join(", ") || "no changes"}`);
      } else {
        setError(
          `Favorites: ${parts.join(", ") || "no changes"}; failed ${failures.length} (${failures.join(", ")})`
        );
      }
    } catch (err) {
      setError(err.response?.data?.message || err.message || "Failed to add to favorites");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApplyLabel = async () => {
    if (!selectedHistory || !requestLabel) {
      setError("Please select a history item and enter a label");
      return;
    }
    ...
    try {
      await presignApi.applyLabelForRequest(selectedHistory.historyId, requestLabel);
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <Container
        header={
          <Header variant="h1" description="View and manage API invocation history">
            SignBridge History
          </Header>
        }
      >
      ...
      >
          ...
          <div style={{
            ...
            <Table
              columnDefinitions={[
                ...
                { id: "historyId", header: "Id", cell: (item) => item.historyId?.substring(0, 8) || "", width: "100px" },
        ...
        <div style={{
          ...
          {selectedItems.length > 1 ? (
            <SpaceBetween size="m">
              <Box>
                <Header variant="h3">{selectedItems.length} requests selected</Header>
                <Box color="text-body-secondary">
                  Multiple requests are selected. Choose a bulk action below.
                  Request details are shown only when a single request is selected.
                </Box>
              </Box>
              <SpaceBetween direction="horizontal" size="s" wrap="wrap">
                ...
                <Button variant="primary" onClick={handleAddToFavorites} loading={loading}>
                  ✔ Add to Favorites
                </Button>
              </SpaceBetween>
            </SpaceBetween>
            ...
            <SpaceBetween size="m">
              <Box>
                <Header variant="h3">
                  Request: {selectedHistory.method} Status:{" "}
                  <span style={{ color: selectedHistory.status === "Complete" ? "green" : "red" }}>
                ...
                <SpaceBetween direction="horizontal" size="s" wrap="wrap">
                  ...
                  <Button onClick={() => copyToClipboard(JSON.stringify(selectedHistory.response || {}, null, 2))}>
                    Copy Response Data
                  </Button>
                  <Button onClick={() => copyToClipboard(JSON.stringify(selectedHistory.responseHeaders || {}, null, 2))}>
                    Copy Response Headers
                  </Button>
                  <Button onClick={() => {
                    let endpoint = "";
                    if (selectedHistory.request) {
                      if (typeof selectedHistory.request === 'object' && selectedHistory.request.endpoint) {
                        endpoint = selectedHistory.request.endpoint;
                      } else if (typeof selectedHistory.request === 'string') {
                        try {
                          const requestObj = JSON.parse(selectedHistory.request);
                    ...
                    if (!endpoint && selectedHistory.endpoint) {
                      endpoint = selectedHistory.endpoint;
                    }
              ...
              <ExpandableSection headerText="Request/Response Details" defaultExpanded>
                <Box padding="s">
                  <SpaceBetween size="m">
                    ...
                    {selectedHistory.responseHeaders && (
                      <Box>
